...
exports.analyzeJD = exports.generateMockTest = exports.submitTestAttempt = exports.updateStreakOnLogin = exports.onUserCreated = void 0;
...
exports.updateStreakOnLogin = functions.https.onCall(async (data, context) => {
    if (!context.auth) {
...
const computePercentile = async (testId, userScore) => {
...
exports.submitTestAttempt = functions.https.onCall(async (data, context) => {
    if (!context.auth)
        return { success: false, message: 'Unauthorized: User must be authenticated.' };
    ...
    try {
        ...
        // Simulate Time Taken
        const timeTaken = Object.values(timings || {}).reduce((a, b) => a + b, 0) || 900;
        ...
        await db.collection('testAttempts').doc(attemptId).set(Object.assign(Object.assign({}, attemptData), { userId: uid }));
        // Award XP
        let xpAward = 25;
        if (percentage > 80)
            xpAward += 50;
        ...
        return Object.assign({ success: true, attemptId }, attemptData);
    }
    catch (error) {
...
exports.generateMockTest = functions.https.onCall(async (data, context) => {
    if (!context.auth)
        return { success: false, message: 'Unauthorized: User must be authenticated.' };
...
exports.analyzeJD = functions.https.onCall(async (data, context) => {
    if (!context.auth)
        return { success: false, message: 'Unauthorized: User must be authenticated.' };
